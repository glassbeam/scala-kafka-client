...
/**
  ...
  */
object AutoPartitionConsumerWithSeekToBeginBoot extends App {
  AutoPartitionConsumer(ConfigFactory.load().getConfig("consumer"))
}

object AutoPartitionConsumerWithSeekToBegin {
  ...
  def apply(config: Config): ActorRef = {
    ...
    system.actorOf(Props(new AutoPartitionConsumerWithSeekToBegin(consumerConf, actorConf)))
  }
}

class AutoPartitionConsumerWithSeekToBegin(
  ...
  consumer ! Subscribe.AutoPartition(List("topic1"),seekToBegin = true)
