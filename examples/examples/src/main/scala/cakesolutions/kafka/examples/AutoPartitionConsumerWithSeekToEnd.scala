...
/**
  ...
  */
object AutoPartitionConsumerWithSeekToEndBoot extends App {
  AutoPartitionConsumer(ConfigFactory.load().getConfig("consumer"))
}

object AutoPartitionConsumerWithSeekToEnd {
  ...
  def apply(config: Config): ActorRef = {
    ...
    system.actorOf(Props(new AutoPartitionConsumerWithSeekToEnd(consumerConf, actorConf)))
  }
}

class AutoPartitionConsumerWithSeekToEnd(
  ...
  consumer ! Subscribe.AutoPartition(List("topic1"),seekToEnd = true)
